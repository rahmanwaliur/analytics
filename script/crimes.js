var max_crimes = function(){
  ...
  if(community_1){
    var community1_profile = new CommunityProfile(community_1, window);

    max = Math.max(max, community1_profile.crimes().year_2012, community1_profile.crimes().year_2013)
  }

  if(community_2){
    var community2_profile = new CommunityProfile(community_2, window);

    max = Math.max(max, community2_profile.crimes().year_2012, community2_profile.crimes().year_2013)
  }
...
var show_crimes = function(crimes, community_id){
  if(!crimes){
    return;
  }
